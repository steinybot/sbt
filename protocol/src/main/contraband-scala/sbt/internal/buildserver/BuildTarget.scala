...
final class BuildTarget private (
  val id: sbt.internal.buildserver.BuildTargetIdentifier,
  val displayName: Option[String],
  val baseDirectory: Option[String],
  val tags: Vector[String],
  val capabilities: sbt.internal.buildserver.BuildTargetCapabilities,
  val languageIds: Vector[String],
  val dependencies: Vector[sbt.internal.buildserver.BuildTargetIdentifier],
  ...
  override def hashCode: Int = {
    37 * (37 * (37 * (37 * (37 * (37 * (37 * (37 * (37 * (37 * (17 + "sbt.internal.buildserver.BuildTarget".##) + id.##) + displayName.##) + baseDirectory.##) + tags.##) + capabilities.##) + languageIds.##) + dependencies.##) + dataKind.##) + data.##)
  }
  override def toString: String = {
    "BuildTarget(" + id + ", " + displayName + ", " + baseDirectory + ", " + tags + ", " + capabilities + ", " + languageIds + ", " + dependencies + ", " + dataKind + ", " + data + ")"
  }
  private[this] def copy(id: sbt.internal.buildserver.BuildTargetIdentifier = id, displayName: Option[String] = displayName, baseDirectory: Option[String] = baseDirectory, tags: Vector[String] = tags, capabilities: sbt.internal.buildserver.BuildTargetCapabilities = capabilities, languageIds: Vector[String] = languageIds, dependencies: Vector[sbt.internal.buildserver.BuildTargetIdentifier] = dependencies, dataKind: Option[String] = dataKind, data: Option[sjsonnew.shaded.scalajson.ast.unsafe.JValue] = data): BuildTarget = {
    new BuildTarget(id, displayName, baseDirectory, tags, capabilities, languageIds, dependencies, dataKind, data)
  }
  def withId(id: sbt.internal.buildserver.BuildTargetIdentifier): BuildTarget = {
    copy(id = id)
  }
  ...
  def withBaseDirectory(baseDirectory: Option[String]): BuildTarget = {
    copy(baseDirectory = baseDirectory)
  }
  def withBaseDirectory(baseDirectory: String): BuildTarget = {
    copy(baseDirectory = Option(baseDirectory))
  }
  def withTags(tags: Vector[String]): BuildTarget = {
    copy(tags = tags)
  }
  def withCapabilities(capabilities: sbt.internal.buildserver.BuildTargetCapabilities): BuildTarget = {
    copy(capabilities = capabilities)
  }
  def withLanguageIds(languageIds: Vector[String]): BuildTarget = {
    copy(languageIds = languageIds)
  }
  def withDependencies(dependencies: Vector[sbt.internal.buildserver.BuildTargetIdentifier]): BuildTarget = {
...
object BuildTarget {
  ...
  def apply(id: sbt.internal.buildserver.BuildTargetIdentifier, displayName: Option[String], baseDirectory: Option[String], tags: Vector[String], capabilities: sbt.internal.buildserver.BuildTargetCapabilities, languageIds: Vector[String], dependencies: Vector[sbt.internal.buildserver.BuildTargetIdentifier], dataKind: Option[String], data: Option[sjsonnew.shaded.scalajson.ast.unsafe.JValue]): BuildTarget = new BuildTarget(id, displayName, baseDirectory, tags, capabilities, languageIds, dependencies, dataKind, data)
  def apply(id: sbt.internal.buildserver.BuildTargetIdentifier, displayName: String, baseDirectory: String, tags: Vector[String], capabilities: sbt.internal.buildserver.BuildTargetCapabilities, languageIds: Vector[String], dependencies: Vector[sbt.internal.buildserver.BuildTargetIdentifier], dataKind: String, data: sjsonnew.shaded.scalajson.ast.unsafe.JValue): BuildTarget = new BuildTarget(id, Option(displayName), Option(baseDirectory), tags, capabilities, languageIds, dependencies, Option(dataKind), Option(data))
}
