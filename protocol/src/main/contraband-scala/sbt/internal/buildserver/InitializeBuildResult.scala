...
final class InitializeBuildResult private (
  ...
  val capabilities: sbt.internal.buildserver.BuildServerCapabilities,
  ...
  override def hashCode: Int = {
    37 * (37 * (37 * (37 * (37 * (37 * (17 + "sbt.internal.buildserver.InitializeBuildResult".##) + displayName.##) + version.##) + bspVersion.##) + capabilities.##) + data.##)
  }
  override def toString: String = {
    "InitializeBuildResult(" + displayName + ", " + version + ", " + bspVersion + ", " + capabilities + ", " + data + ")"
  }
  private[this] def copy(displayName: String = displayName, version: String = version, bspVersion: String = bspVersion, capabilities: sbt.internal.buildserver.BuildServerCapabilities = capabilities, data: Option[sjsonnew.shaded.scalajson.ast.unsafe.JValue] = data): InitializeBuildResult = {
  ...
  def withCapabilities(capabilities: sbt.internal.buildserver.BuildServerCapabilities): InitializeBuildResult = {
...
object InitializeBuildResult {
  ...
  def apply(displayName: String, version: String, bspVersion: String, capabilities: sbt.internal.buildserver.BuildServerCapabilities, data: Option[sjsonnew.shaded.scalajson.ast.unsafe.JValue]): InitializeBuildResult = new InitializeBuildResult(displayName, version, bspVersion, capabilities, data)
  def apply(displayName: String, version: String, bspVersion: String, capabilities: sbt.internal.buildserver.BuildServerCapabilities, data: sjsonnew.shaded.scalajson.ast.unsafe.JValue): InitializeBuildResult = new InitializeBuildResult(displayName, version, bspVersion, capabilities, Option(data))
}
